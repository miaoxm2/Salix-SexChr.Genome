...
#TODO: Find the alleles that homo in all females and heter in all males. and the alleles of female belong to two alles of males
#USAGE:
...
while (my $line = <F1>) {
    ...
    for (my $i = $pos+$mnum*2; $i < $pos+$mnum*4; $i=$i+2) {
        my ($gt1,$gt2) = (split/[\/|]/, $fields[$i])[0,1];
        if ($gt1 ne $gt2) {
            if (exists $allele{$gt1} || exists $allele{$gt2} ) { 
                $is_unique++;
            }
        }
